...
export const GEAR = {
  ...
  focus: {
    ...
    tiers: [
      {},
      { cost: 80, mods: { cdr: +3, damage: +3 } },
      { cost: 190, mods: { cdr: +7, damage: +8 } },
      { cost: 400, mods: { cdr: +12, damage: +14 } },
    ],
  },
  ...
  boots: {
    ...
    tiers: [
      {},
      { cost: 50, mods: { moveSpeed: +4, evasion: +3 }, mult: { taken: -4 } },
      { cost: 130, mods: { moveSpeed: +8, evasion: +7 }, mult: { taken: -7 } },
      { cost: 280, mods: { moveSpeed: +13, evasion: +12 }, mult: { taken: -11 } },
    ],
  },
}

/**
 * `cdr` and `moveSpeed` from gear add into the same stats the attributes feed,
 * and `deriveStats` re-applies the global ceilings afterwards — so a tier 3
 * staff cannot push a mage past the cooldown cap.
 */
