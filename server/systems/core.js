...
function spendMoveToken(player, now) {
  // Agility shortens the cooldown, so the refill rate is per-player. The
  // profile writes it; the fallback covers the tick before the profile exists.
  const cooldown = player.moveCooldownMs ?? MOVE_COOLDOWN_MS

  const elapsed = now - player.moveTokensAt
  player.moveTokensAt = now
  player.moveTokens = Math.min(MOVE_BURST_TILES, player.moveTokens + elapsed / cooldown)
